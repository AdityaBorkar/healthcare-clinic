...
- Validation schemas come from the Aspen OS modules (`@aspen-os/healthcare`, `@aspen-os/masters`, `@aspen-os/management`, `@aspen-os/platform/server`) — valibot, not zod. Procedures use the module's schema directly as `.input(...)`. Custom schemas live in their procedure module only where no Aspen equivalent exists (CPT/NoShow in `admin`/`appointments`, workspace users in `users`); forms needing custom validation define it locally, otherwise they reuse the module schema. Note `src/env.ts` itself uses `zod` via `@t3-oss/env-core`.
