...
export async function MinioContainer({
  ...
  const MINIO_CONSOLE_PORT = Number.parseInt(config.require("console_port"), 10);

  const image = new docker.RemoteImage("minio-image", { name: "minio/minio:latest" }, { provider });

  const volume = new docker.Volume("minio-data", { labels: GROUP_LABELS }, { provider });

  const container = new docker.Container(
    "minio-server",
    {
      command: ["server", "/data", "--console-address", ":9001"],
      envs: [`MINIO_ROOT_USER=${MINIO_ROOT_USER}`, `MINIO_ROOT_PASSWORD=${MINIO_ROOT_PASSWORD}`],
      image: image.name,
      labels: GROUP_LABELS,
