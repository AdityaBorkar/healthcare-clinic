...
export function SettingsHeader() {
  ...
  return (
    <header className="sticky top-0 z-10 border-stone-border border-b bg-white/80 backdrop-blur-md">
            ...
            {breadcrumbs.map((crumb, index) => {
              ...
              return (
                <BreadcrumbItem className="min-w-0" key={crumb.href}>
                  ...
                  {isLast ? (
                    <BreadcrumbPage className="min-w-0 truncate font-medium text-ink-black text-sm">
                      {crumb.label}
                    </BreadcrumbPage>
                  ) : (
                    <BreadcrumbLink
                      render={
                        <Link
                          className="block truncate rounded px-1 py-0.5 text-warm-gray transition-colors hover:text-ink-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-signal/30"
                          to={crumb.href}
                        />
                      }
                    >
